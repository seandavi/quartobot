...
it](install.qmd#verify-quarto-can-find-it).
...
- [Install reference](install.qmd) — every install method, what to
  use when.
- [Validate a manuscript](validate-manuscript.qmd) — the pre-flight
  check that catches most of these before the render fails.
- [Resolve a single citation](resolve-single-citation.qmd) — the
