...
- **`quartobot resolve`** — the citation resolver. Reads
  ...
  [resolve a single citation](resolve-single-citation.qmd).
...
- **`quartobot mcp`** — opt-in extra. Starts an MCP stdio server
  ...
  Install with `quartobot[mcp]`. See [the MCP server page](mcp.qmd).
- **`quartobot scan`** / **`quartobot validate`** — pre-flight
  ...
  [validate a manuscript](validate-manuscript.qmd).
...
- [First manuscript (15 min)](first-manuscript.qmd) — the same flow as a guided end-to-end walk-through if you'd rather follow one path than pick between three.
