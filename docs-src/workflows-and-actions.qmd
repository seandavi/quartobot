...
- [CLI reference](cli.qmd) — `quartobot use github-ci` is the
...
- [Differences from manubot](differences-from-manubot.qmd) — design
  rationale on the lean-default and the retention policy.
- [Troubleshooting](troubleshooting.qmd) — when the canonical path
  breaks.
