...
etc.), see [Workflows and actions](workflows-and-actions.qmd) in
...
- [CLI reference](cli.qmd) — every command, every flag.
- [MCP server](mcp.qmd) — point Claude Desktop, Codex, or Gemini
  Code Assist at `quartobot mcp` so an agent can call the
  `resolve_citation` tool while you draft.
- [Migrating from manubot](migrating-from-manubot.qmd) — if you have
  a manubot manuscript and want to translate the layout.
- [Design](design.qmd) — the architecture decisions and prior art.
