...
`references.resolved.bib` (auto-resolved bibliography) and `_freeze/`
...
etc.), resolves each through `manubot.cite`, converts to BibLaTeX via
pandoc, and writes `references.resolved.bib` with the user's original
key as the citation key. Then pandoc runs in the normal way and
pandoc-citeproc reads `references.resolved.bib` and `references.bib`
together.
