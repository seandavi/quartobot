...
`_book/`, `references.resolved.bib`, and `_freeze/` are all generated
by the render step. The `.gitignore` excludes them — they regenerate
from `_quarto.yml`, the chapter files, and `references.bib`.
...
bibliography:
  ...
  - references.resolved.bib     # auto-resolved by quartobot
```
